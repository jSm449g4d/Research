...
#    mod=Conv2D(64,3,padding="same",activation="relu")(mod)
    mod_1=mod
    mod_1=Conv2D(128,3,2,padding="same",activation="relu")(mod_1)
#    mod_1=Conv2D(128,3,padding="same",activation="relu")(mod_1)
    mod_2=mod_1
    mod_2=Conv2D(256,3,2,padding="same",activation="relu")(mod_2)
...
def test():
    ...
    os.makedirs(os.path.join("./", 'out2'),exist_ok=True)
    ...
    for i,dataX in enumerate(dataset):
        ...
        tf2img(predY,os.path.join("./", 'out2'),name=os.path.basename(dataX))
