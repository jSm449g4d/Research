...
def GEN(input_shape=(128,128,3,)):
    ...
    # modB=mod
    # #AE_H
    # modB=Conv2D(16,8,4,padding="same",kernel_regularizer=regularizers.l1(0.0))(modB)
    # modB=Conv2D(16,8,4,padding="same",kernel_regularizer=regularizers.l1(0.0))(modB)
    # #AE_M
    # modB=Flatten()(modB)
    # modB=Dense(32,kernel_regularizer=regularizers.l1(0.005))(modB)
    # modB=LayerNormalization()(modB)
    # modB=Dense(16*16*16,kernel_regularizer=regularizers.l1(0.0))(modB)
    # modB=Reshape((16,16,16))(modB)
    # #AE_T
    # modB=Bottleneck(16)(modB)
    # modB=UpSampling2D(2)(modB)
    # modB=Bottleneck(16)(modB)
    # modB=Bottleneck(16)(modB)
    # modB=UpSampling2D(2)(modB)
    # modB=Bottleneck(16)(modB)
    ...
    # modB=Bottleneck(16)(modB)
    # modB=Bottleneck(16)(modB)
    # modB=Conv2D(3,1,padding="same")(modB)
    
    # modC=mod
    # #AE_H
    # modC=Flatten()(modC)
    # #AE_M
    # modC=Dense(8,kernel_regularizer=regularizers.l1(0.005))(modC)#,activation="relu"
    # modC=LayerNormalization()(modC)
    # modC=Dense(128*128*3,kernel_regularizer=regularizers.l1(0.0))(modC)
    # modC=Reshape((128,128,3))(modC)
    #AE_T
    
    #mod=tf.concat([modA,modB,modC],axis=-1)#,modB
#    mod=Conv2D(16,8,padding="same",kernel_regularizer=regularizers.l1(0.0),activation="relu")(mod)
    
    mod=Conv2D(32,5,4,padding="same",kernel_regularizer=regularizers.l1(0.0))(mod)
    mod=Bottleneck(32)(mod)
    mod=UpSampling2D(4)(mod)
    mod=Conv2D(8,5,padding="same",kernel_regularizer=regularizers.l1(0.0))(mod)
    mod=Bottleneck(8)(mod)
    mod=Bottleneck(8)(mod)    

    mod=Conv2D(3,5,padding="same",kernel_regularizer=regularizers.l1(0.0))(mod)
...
def main():
    ...
    model.summary()
    cbks=[keras.callbacks.TensorBoard(log_dir='logsLasso7', histogram_freq=1)]
    cbks=[]
    
    model.fit(x_train, y_train,epochs=20,validation_data=(x_test, y_test),callbacks=cbks)
    model.save('modelLasso7.h5')
